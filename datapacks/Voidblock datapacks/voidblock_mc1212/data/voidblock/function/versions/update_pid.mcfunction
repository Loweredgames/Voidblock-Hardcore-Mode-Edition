...
execute as @a[scores={PID_1_mc1212=250..}] run scoreboard objectives add SUBPID_1_mc1212 dummy


##SUBPID END (QUANDO FINISCE IL PID AGGIUNGERE IL SUBPID O IL PID MA SOLO NEL PRIMO SUBPID. GUARDARE SEMPRE LA SCORE ALLA FINE DELLA FUNZIONE, ANCHE NEL SUBPID DENTRO LA FUNZIONE. DISABILITA SE NON C'È NESSUN SUBPID)
#>execute as @a[scores={SUBPID_1_mc1212=1015..}] run scoreboard objectives add SUBPID_2_mc1212 dummy (aggiungere per il futuro il 2)


##SETUP UPDATE SUBPID VERSION (LASCIARE SEMPRE LA SCOREBOARD E SPOSTARE QUANDO DIVENTA LEGACY)
scoreboard players add @a SUBPID_1_mc1212 1
#scoreboard players add @a SUBPID_2_mc1212 1


##SCOREBOARD START PID VERSION (LASCIARE SEMPRE COSI, MODIFICARE SE LA CONVERSIONE E PIU CORTA O PIU LUNGHA.)
execute as @a[scores={Voidblock_final=351}] run scoreboard players set @a PID_1_mc1212 -250
execute as @a[scores={PID_1_mc1212=5..100}] run gamemode spectator @a
execute as @a[scores={PID_1_mc1212=5}] run time set 0
execute as @a[scores={PID_1_mc1212=5}] run gamerule doImmediateRespawn true
execute as @a[scores={PID_1_mc1212=5..200}] run effect give @a blindness 5 0 true
execute as @a[scores={PID_1_mc1212=5..200}] run effect give @a night_vision 5 0 true
execute as @a[scores={PID_1_mc1212=200}] run gamerule doImmediateRespawn false
...
execute as @a[scores={PID_1_mc1212=201}] run advancement revoke @a only minecraft:nether/root
